...
import { handleStartDraw, 
    handleDraw, 
    handleStopDraw, 
    handleUndoDraw, 
    handleClearCanvas 
} from './canvasEventHandler.js';
import { handleSendMessage } from './chatEventHandler.js';
import { updatePlayerList } from './playerListUpdateHandler.js';
...
io.on('connection', function (socket) {
    ...
    socket.on('active_room', (data) => {
        ...
        if (!roomError) {
            ...
            if (!Object.keys(roomStore[roomId]).includes(username)) {
                ...
                if (!roomStore[roomId]['gameState'] && roomStore[roomId]['userList'].length >= 2) {
            ...
            if (roomStore[roomId]["currentDrawer"][0] === username) {
                ...
                io.to(socket.id).emit('drawer', true, roomStore[roomId]['answer'][0]);
            }
            //User is not the current Drawer
            else {
                io.to(socket.id).emit('drawer', false, roomStore[roomId]['answer'][0]);
            }
    ...
    socket.on('disconnect', (_) => {
        if (socketStore[socket.id]) {
            ...
            // update game state
            roomStore[roomId]['gameState'] = roomStore[roomId]['userList'].length >= 2 && roomStore[roomId]['userList'].length <= 8;

            // update current drawer
            if (roomStore[roomId]['currentDrawer'] === username) {
                let randomUser = getRandomInt(0, roomStore[roomId]['userList'].length);
                ...
                if (roomStore[roomId]['userList'].length === 0) {
                    ...
                    roomStore[roomId]['currentDrawer'] = roomStore[roomId]['userList'][randomUser];
                }
            }
