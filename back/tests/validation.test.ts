...
describe('roomCreateRequest', () => {
    it('accepts a request the UI could actually produce', () => {
        expect(roomCreateRequest.safeParse(validCreateRequest).success).toBe(
            true,
        );
    });
    ...
    it('bounds the round count', () => {
        expect(
            roomCreateRequest.safeParse({ ...validCreateRequest, rounds: 0 })
                .success,
        ).toBe(false);
        expect(
...
describe('joinRoomRequest', () => {
    ...
    it('rejects extra positional arguments', () => {
        expect(
            joinRoomRequest.safeParse([validRoomId, 'Alice', '', 'extra'])
                .success,
        ).toBe(false);
    });
});
...
describe('startDrawingRequest', () => {
    ...
    it('rejects a colour that is not a hex value', () => {
        for (const color of [
            ...
            expect(
                startDrawingRequest.safeParse([validRoomId, point, color, 12])
                    .success,
            ).toBe(false);
        }
    });
    ...
    it('bounds the brush size', () => {
        expect(
            startDrawingRequest.safeParse([validRoomId, point, '#000000', 0])
                .success,
        ).toBe(false);
        expect(
            startDrawingRequest.safeParse([
